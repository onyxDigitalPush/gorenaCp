            ...
            <div class="modal-dialog modal-sm glassmorphism">
              <div class="modal-content glassmorphism ">
                <div class="modal-body">
                    <h3 style="color:black;"><?php echo "Selecciona Subempresa"?></h3><br>
                        <form method="post" name="sbe">
                            <select name="llista" class="mi-select">
                                <option hidden selected value><?php echo $dto->getCampPerIdCampTaula("subempresa",$idsubemp,"nom"); ?></option>
                            <?php
                            foreach($sbe as $data)
                            {
                                echo '<option value="'.$data["idsubempresa"].'">'.$data["nom"].'</option>';                                
                            }
                            ?>
                            </select>
                            <br><br><button type="button" class="btn_modal" data-dismiss="modal"><?php echo $dto->__($lng,"Tornar"); ?> <span class='glyphicon glyphicon-remove'></span></button>
                    <button type="button" class="btn_modal" data-dismiss="modal" onclick="actualitzaCampTaula('empleat','idsubempresa',<?php echo $id; ?>,'',sbe.llista.value);"><?php echo $dto->__($lng,"Assignar"); ?> <span class='glyphicon glyphicon-ok'></span></button>
                   </form> 
                </div>
              </div>
            </div>
