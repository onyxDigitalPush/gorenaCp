...
<html>
    ...
    $id = intval($_GET['id']);
    $role = $dto->mostraRols($dto->mostraIdEmpresaPerIdEmpleat($id),$dto->esMaster($id));
    ?>
            ...
            <div class="modal-dialog modal-sm glassmorphism">
              <div class="modal-content glassmorphism">
                <div class="modal-body">
                    <h3 style="color:black;"><?php echo "Selecciona Rol"?></h3><br>
                        <form method="post" name="rols">
                            <select name="llista">
                                <option hidden selected value><?php echo $dto->mostraNomRolPerIdEmp($id); ?></option>
                            <?php
                            foreach($role as $data)
                            {
                                echo '<option value="'.$data["idrol"].'">'.$data["nom"].'</option>';                                
                            }
                            ?>
                            </select>
                            <br><br><button type="button" class="btn_modal" data-dismiss="modal"><?php echo $dto->__($lng,"Tornar"); ?></button>
                    <button type="button" class="btn_modal" onclick="assignaRol(<?php echo $id; ?>,rols.llista.value)"><?php echo $dto->__($lng,"Assignar"); ?></button>
                   </form> 
                </div>
              </div>
            </div>
