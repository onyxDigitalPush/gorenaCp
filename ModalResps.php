...
<html>
    ...
    $id = intval($_GET['id']);
    $idresp = intval($_GET['idresp']);
    $noms = $dto->mostraRespEmp($dto->mostraIdEmpresaPerIdEmpleat($id));
    include 'Conexion.php';



    $consulta = "SELECT * FROM empleat WHERE isEncargado = 1";
...
<style>
    ...
    <center>
            ...
            <div class="modal-dialog modal-sm glassmorphism">
              <div class="modal-content glassmorphism">
                <div class="modal-body">
                    <h3 style="color: black;"><?php echo "Selecciona Responsable"?></h3><br>
                    <form method="POST" action="process_encargado.php">
      
                                  <select name="idresp" id="idresp" class="mi-select">
                <?php
                while ($fila = mysqli_fetch_assoc($resultado)) {
                  var_dump($fila);
                  echo '<option name="test" value="        ' . $fila['idempleat'] . '      "        >        ' . $fila['nom'] . '       </option>'; 
                }
                ?>
              </select>
        ...
        <button type="submit" class="btn_modal">Asignar</button>
    </form>
                </div>
              </div>
            </div>
